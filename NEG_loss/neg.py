...
class NEG_loss(nn.Module):
    def __init__(self, num_classes, embed_size):
        ...
        self.out_embed.weight = t.nn.Parameter(t.FloatTensor(self.num_classes, self.embed_size).uniform_(-1, 1))

        self.in_embed = nn.Embedding(self.num_classes, self.embed_size)
        self.in_embed.weight = t.nn.Parameter(t.FloatTensor(self.num_classes, self.embed_size).uniform_(-1, 1))
    ...
    def forward(self, input_labels, out_labels, num_sampled):
        ...
        sum_log_sampled = t.bmm(noise, input.unsqueeze(2)).sigmoid().log().sum(1).squeeze(1)
